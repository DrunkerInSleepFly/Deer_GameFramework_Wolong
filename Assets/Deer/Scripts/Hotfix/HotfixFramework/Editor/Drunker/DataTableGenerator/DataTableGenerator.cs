...
using UnityEngine;
    ...
    public sealed class DataTableGenerator
    {
        ...
        public static DataTableProcessor CreateDataTableProcessor(string dataTableName)//GB2312
        {
        ...
        public static void GenerateByteDataFile(DataTableProcessor dataTableProcessor, string dataTableName)
        {
            string binaryDataFileName = Utility.Path.GetRegularPath(Path.Combine(HotfoxDataTablePath, dataTableName + ".bytes"));
            if (!dataTableProcessor.GenerateByteDataFile(binaryDataFileName) && File.Exists(binaryDataFileName))
            {
                File.Delete(binaryDataFileName);
            }
        }

        public static void CopyTxtFiles(DataTableProcessor dataTableProcessor, string dataTableName)
        {
            string[] fileNames = Directory.GetFiles(DataTablePath, "*.txt", SearchOption.AllDirectories);
            foreach (string fileName in fileNames)
            {
                string sourcePath = Path.Combine(fileName);

                Debug.Log(sourcePath);
                string destFileName = Path.Combine(HotfoxDataTablePath, sourcePath.Substring(DataTablePath.Length + 1));
        ...
        private static string GenerateDataTableParser(DataTableProcessor dataTableProcessor)
        {
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                else
                {
                    ...
                    if (languageKeyword.StartsWith("List"))
                    {
                        if (languageKeyword == "List<Vector2>")
                        {
                            stringBuilder.AppendFormat("            {0} = DataTableExtension.ParseListVector2(columnStrings[index++]);", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                        }
                        else if (languageKeyword == "List<Vector3>")
                        {
                            stringBuilder.AppendFormat("            {0} = DataTableExtension.ParseListVector3(columnStrings[index++]);", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                        }
                        else if (languageKeyword == "List<int>")
                        {
                            stringBuilder.AppendFormat("            {0} = DataTableExtension.ParseListInt(columnStrings[index++]);", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                        }
                        else if (languageKeyword == "List<float>")
                        {
                            stringBuilder.AppendFormat("            {0} = DataTableExtension.ParseListFloat(columnStrings[index++]);", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                        }

                    }
                    else if (languageKeyword.StartsWith("Dictionary"))
                    {
                        if (languageKeyword == "Dictionary<int, int>")
                        {
                            stringBuilder.AppendFormat("            {0} = DataTableExtension.ParseDictionaryIntAndInt(columnStrings[index++]);", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                        }
                        else if (languageKeyword == "Dictionary<int, string>")
                        {
                            stringBuilder.AppendFormat("            {0} = DataTableExtension.ParseDictionaryIntAndString(columnStrings[index++]);", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                        }
                        else if (languageKeyword == "Dictionary<string, string>")
                        {
                            stringBuilder.AppendFormat("            {0} = DataTableExtension.ParseDictionaryStringAndString(columnStrings[index++]);", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                        }
                        else if (languageKeyword == "Dictionary<string, int>")
                        {
                            stringBuilder.AppendFormat("            {0} = DataTableExtension.ParseDictionaryStringAndInt(columnStrings[index++]);", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                        }
                        else if (languageKeyword == "Dictionary<int, float>")
                        {
                            stringBuilder.AppendFormat("            {0} = DataTableExtension.ParseDictionaryIntAndFloat(columnStrings[index++]);", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                        }
                    }
                    else
                    {
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                else if (languageKeyword.StartsWith("List") && (languageKeyword == "List<Vector2>" || languageKeyword == "List<Vector3>" ||
                    languageKeyword == "List<int>" || languageKeyword == "List<float>"))
                {
                    if (languageKeyword == "List<Vector2>")
                    {
                        stringBuilder.AppendFormat("                    {0} = binaryReader.ReadListVector2();", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                    }
                    else if (languageKeyword == "List<Vector3>")
                    {
                        stringBuilder.AppendFormat("                    {0} = binaryReader.ReadListVector3();", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                    }
                    else if (languageKeyword == "List<int>")
                    {
                        stringBuilder.AppendFormat("                    {0} = binaryReader.ReadListInt();", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                    }
                    else if (languageKeyword == "List<float>")
                    {
                        stringBuilder.AppendFormat("                    {0} = binaryReader.ReadListFloat();", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                    }
                }
                else if (languageKeyword.StartsWith("Dictionary"))
                {
                    if (languageKeyword == "Dictionary<int, int>")
                    {
                        stringBuilder.AppendFormat("                    {0} = binaryReader.ReadDictionaryIntAndInt();", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                    }
                    else if (languageKeyword == "Dictionary<int, string>")
                    {
                        stringBuilder.AppendFormat("                    {0} = binaryReader.ReadDictionaryIntAndString();", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                    }
                    else if (languageKeyword == "Dictionary<string, string>")
                    {
                        stringBuilder.AppendFormat("                    {0} = binaryReader.ReadDictionaryStringAndString();", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                    }
                    else if (languageKeyword == "Dictionary<string, int>")
                    {
                        stringBuilder.AppendFormat("                    {0} = binaryReader.ReadDictionaryStringAndInt();", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                    }
                    else if (languageKeyword == "Dictionary<int, float>")
                    {
                        stringBuilder.AppendFormat("                    {0} = binaryReader.ReadDictionaryIntAndFloat();", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                    }
                }
                else
                {
