...
using UnityEngine;
    ...
    public sealed partial class DataTableProcessor
    {
        ...
        public bool GenerateByteDataFile(string outputFileName)
        {
        ...
        public bool GenerateTxtDataFile(string outputFileName)
        {
            if (string.IsNullOrEmpty(outputFileName))
            {
                throw new GameFrameworkException("Output file name is invalid.");
                ...
                using (BinaryWriter binaryWriter = new BinaryWriter(memoryStream, Encoding.UTF8))
                {
                    for (int rawColumn = 0; rawColumn < RawColumnCount; rawColumn++)
                    {
                        ...
                        catch
                        {
                            ...
                            {
                                //Debug.Log(m_DataProcessor[rawColumn].IsId);
                                //Debug.Log(string.IsNullOrEmpty(GetDefaultValue(rawColumn)));
                                //Debug.Log(rawColumn);
                                //Debug.Log(GetDefaultValue(rawColumn));
